...
/// This test writes to a fallback credentials file in BLUEPRINTLM_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire BLUEPRINTLM_HOME through the code.
#[serial(codex_home)]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn streamable_http_with_oauth_round_trip() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("BLUEPRINTLM_HOME", temp_home.path().as_os_str());
