...
If you already have the combined `AGENTS.md` contents on hand, you can pass them straight into the CLI when starting a session. Use `--project-doc` on `blueprintlm-codex start-session` to inject the text instead of letting Codex read from disk. Passing `-` reads from stdin, which is convenient for piping:
